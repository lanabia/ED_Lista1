...
Questão 3 - Lista Duplamente Encadeada

Implemente um TAD para representar uma lista duplamente encadeada e ordenada capaz de
armazenar valores inteiros. Esse TAD deverá conter funções para:
...
Implemente um programa que cria uma variável do TAD lista duplamente encadeada e
ordenada, criado anteriormente, e utiliza cada uma de suas funções.
...
public class Main {
	...
	public static Lista lst_insere(Lista lst, int val) {
		...
		while (atual.info < val) {
			if (atual.next == null) {
				atual.next = new Lista(val,null,atual);
				return lst;
			}
		...
		if (count == 1)
			return new Lista(val, lst,anterior);
		else{
			anterior.next = new Lista(val, atual,anterior);
